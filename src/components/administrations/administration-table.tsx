...
const admins: Administrator[] = [
  {
    name: "Ali Khan",
    email: "ali.khan@pizzahouse.internal",
    role: "Restaurant Admin",
    locations: "All Locations (4)",
  ...
  {
    name: "Sara Malik",
    email: "sara.malik@pizzahouse.internal",
    role: "Kitchen Supervisor",
    locations: "Downtown Pizza House (#102)",
  ...
  {
    name: "Zainab Hassan",
    email: "zainab.hassan@pizzahouse.internal",
    role: "Finance & Billing",
    locations: "Westside Kitchen (#105)",
    ...
    <Card className="overflow-hidden">
      <div className="flex flex-col gap-3 border-b bg-surface-low p-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative w-full sm:w-80">
          ...
          <Input className="h-9 pl-9" placeholder="Search administrators..." />
        </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[850px] text-left">
          ...
          <tbody className="divide-y">
            {admins.map((admin) => (
              <tr key={admin.email} className="hover:bg-muted">
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <Link
                        href={`/administrations/${admin.name.toLowerCase().replace(" ", "-")}`}
                        className="text-body-sm font-semibold hover:text-primary"
                      >
                        {admin.name}
                      </Link>
                ...
                <td className="px-6 py-4 text-body-sm text-muted-foreground">
                  {restaurantScoped ? (
                    <span
                      className={`rounded-full px-2 py-1 text-xs font-semibold ${statusClass[admin.status]}`}
                    >
                      {admin.status}
                    </span>
                  ) : (
                ...
                <td className="px-6 py-4 text-right">
                  <RowActionsMenu
                    ...
                    actions={[
                      ...
                      { label: "Edit", href: "/administrations/invite" },
                      {
                        label:
...
export function AdminEmptyIcon() {
  return (
    <div className="mb-6 flex h-20 w-20 items-center justify-center rounded-full border bg-surface text-muted-foreground">
