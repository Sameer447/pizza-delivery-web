...
export const restaurantRows: RestaurantRow[] = [
  {
    name: "Pizza House",
    slug: "@pizza-house",
...
export function RestaurantTable({
  ...
  const [selected, setSelected] = useState<string[]>(
    selection ? [restaurantRows[1].name] : [],
  );
  ...
  return (
    <Card className="overflow-hidden">
      <div className="flex flex-col gap-3 border-b bg-surface-low p-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative w-full sm:w-72">
          <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
          <Input className="h-9 pl-9" placeholder="Search restaurants..." />
        </div>
        ...
        <table className="w-full min-w-[900px] text-left">
          <thead className="border-b bg-surface-high text-muted-foreground">
            <tr>
              ...
              <th className="type-table-header px-4 py-3">Restaurant</th>
              <th className="type-table-header px-4 py-3">Contact</th>
              <th className="type-table-header px-4 py-3">Location</th>
              <th className="type-table-header px-4 py-3 text-right">Orders</th>
              <th className="type-table-header px-4 py-3 text-right">
                Revenue
              </th>
              <th className="type-table-header px-4 py-3 text-center">
                Status
              </th>
              <th className="type-table-header px-4 py-3">Created</th>
            ...
            {restaurantRows.map((restaurant) => (
              ...
              >
                <td className="px-4 py-3">
                  <input
                    type="checkbox"
                    checked={selected.includes(restaurant.name)}
                    onChange={() => toggle(restaurant.name)}
                    aria-label={`Select ${restaurant.name}`}
                  />
                </td>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded border bg-surface-low text-muted-foreground">
                ...
                <td className="px-4 py-3 text-right text-body-sm font-medium">
                  {restaurant.orders}
                </td>
                <td className="px-4 py-3 text-right text-body-sm font-medium">
                  {restaurant.revenue}
                ...
                <td className="px-4 py-3 text-center">
                  <span
                    className={cn(
                      "inline-flex rounded-full border px-2 py-1 text-[11px] font-bold uppercase tracking-wide",
                      statusClass[restaurant.status],
                    )}
                  >
                    {restaurant.status}
                  </span>
...
export function RestaurantSummary() {
  const summary = [
    { label: "Total", value: "24", icon: Store, color: "text-info bg-info/10" },
    {
      label: "Active",
      value: "20",
      icon: CheckCircle2,
    ...
    <div className="grid grid-cols-2 gap-4 xl:grid-cols-4">
      {summary.map(({ label, value, icon: Icon, color }) => (
        <Card key={label} className="flex items-center gap-4 p-4">
          <div className={cn("rounded-[var(--radius)] p-2.5", color)}>
